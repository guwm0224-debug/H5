...
(function () {
    ...
    class UIController {
        ...
        updateState(patch = {}) {
            ...
            this.broadcastStateChange(prevState);
        }
        ...
        createObserver() {
            ...
            const checkVideoPresence = () => {
                ...
                if (hasVideo !== this.uiState.hasActiveVideo) {
                    const prevState = { ...this.uiState };
                    this.uiState.hasActiveVideo = hasVideo;
                    this.applyVisibilityRule();
                    this.broadcastStateChange(prevState);
                }
            };
            ...
            this.observer = new MutationObserver(() => checkVideoPresence());
        ...
        applyVisibilityRule() {
            if (!this.uiState.hasActiveVideo) {
                this.uiState.isPanelVisible = false;
            }
        }

        broadcastStateChange(prevState) {
            this.subscribers.forEach(callback => {
                try {
                    callback({ ...this.uiState }, prevState);
                } catch (error) {
                    console.error('UIController subscriber error:', error);
                }
            });
        }
    }
    ...
    class ModuleRegistry {
        ...
        destroyAll() {
            this.list().forEach(name => this.unload(name));
        }
    ...
    class VideoSpeedController {
        constructor() {
            ...
            this.pluginSettings = this._loadPluginSettings();
            this.ui = null;
        ...
        _getInitialState() {
            return {
                ...
                isMuted: false,
                isAudioMode: false,
                isStreamingOptimizationEnabled: GM_getValue('streamingOptimizationEnabled', true),
                currentVideo: null,
                previousVideo: null,
        ...
        async _initialize() {
            try {
                ...
                if (this.state.lifecycle) {
                    ...
                    this.state.lifecycle.initTimestamp = Date.now();
                }
        ...
        _startWithRetry() {
            ...
            const tryStart = () => {
                if (this.state.lifecycle) {
                    this.state.lifecycle.startTriggered = true;
                    this.state.lifecycle.lastStartAttempt = Date.now();
                    this.state.lifecycle.retryCount = retryCount;
                }
                try {
                    this._start();
                } catch (error) {
        ...
        _start() {
            if (!this.ui) {
                this.ui = new EnhancedUIManager(this, this.uiController);
            }

            let controlsInitialized = false;

            this.uiController.onVideoChange((newState) => {
                if (newState.hasActiveVideo && !controlsInitialized) {
            ...
            this.uiController.createObserver();
            if (this.state.lifecycle) {
                this.state.lifecycle.uiObserverActive = !!this.uiController.observer;
            }

            Object.entries(this.managers).forEach(([name, instance]) => {
                this.moduleRegistry.register(name, instance);
            });
            ...
            if (this.state.isStreamingOptimizationEnabled) {
                this.managers.streamingMedia.initialize(); // 新增
            } else {
                console.log('流媒体优化已关闭，跳过流媒体模块初始化');
            }

            if (this.state.lifecycle) {
                this.state.lifecycle.modulesReady = true;
            }
            ...
            setTimeout(() => {
                try {
                    ...
                    this._notifyPerformanceModulesReady();
         ...
         * 性能模块准备就绪后通知相关模块
         */
        _notifyPerformanceModulesReady() {
            if (this.managers.video && typeof this.managers.video.onPerformanceModuleReady === 'function') {
                this.managers.video.onPerformanceModuleReady(
                    this.managers.performanceAnalyzer,
                    this.managers.performanceMonitor
                );
            }

            if (this.managers.streamingMedia && typeof this.managers.streamingMedia.onPerformanceAnalyzerReady === 'function') {
                this.managers.streamingMedia.onPerformanceAnalyzerReady();
            }
        }
        ...
        _setupEventListeners() {
            ...
            if (this.state.lifecycle) {
                this.state.lifecycle.listenersReady = true;
            }
        }
        ...
        _main() {
            if (this.state.lifecycle) {
                this.state.lifecycle.controlsAttempted = true;
            }

            if (this.state.isControlsCreated) {
                if (this.state.lifecycle) {
                    this.state.lifecycle.controlsReady = true;
                }
        ...
        _cleanup() {
            if (this.managers.video && typeof this.managers.video.cleanup === 'function') {
                this.managers.video.cleanup();
            }
            if (this.managers.audio && typeof this.managers.audio.cleanup === 'function') {
                this.managers.audio.cleanup();
            }
            if (this.managers.storage && typeof this.managers.storage.cleanupExpiredRecords === 'function') {
        ...
        _registerMenuCommands() {
            if (typeof GM_registerMenuCommand !== 'function') {
                console.warn('Tampermonkey菜单注册未启用：GM_registerMenuCommand 不可用');
                return;
            }

            GM_registerMenuCommand('故障诊断', () => this._runDiagnostics());
            GM_registerMenuCommand('插件设置', () => this._openSettingsPanel());
            const streamingLabel = `流媒体优化：${this.state.isStreamingOptimizationEnabled ? '开启' : '关闭'}`;
            GM_registerMenuCommand(streamingLabel, () => this._toggleStreamingOptimization());
        }

        _toggleStreamingOptimization() {
            const enabled = !this.state.isStreamingOptimizationEnabled;
            this.state.isStreamingOptimizationEnabled = enabled;
            GM_setValue('streamingOptimizationEnabled', enabled);

            if (enabled) {
                if (this.managers.streamingMedia) {
                    this.managers.streamingMedia.initialize();
                }
                this._notifyStreamingStatus('已开启流媒体优化', '正在应用缓存与自适应策略');
            } else {
                if (this.managers.streamingMedia && typeof this.managers.streamingMedia.destroy === 'function') {
                    this.managers.streamingMedia.destroy();
                }
                this._notifyStreamingStatus('已关闭流媒体优化', '已停止流媒体增强处理');
            }

        }

        _notifyStreamingStatus(title, text) {
            try {
                if (typeof GM_notification !== 'undefined') {
                    GM_notification({ title: `H5媒体播控器 - ${title}`, text, timeout: 4000 });
                    return;
                }
            } catch (error) {
                console.warn('GM_notification 调用异常，回退到 alert', error);
            }
        ...
        _runDiagnostics() {
            const steps = [];
            const lifecycleSnapshot = { ...(this.state.lifecycle || {}) };
            const runStep = (label, action) => {
                const start = performance.now();
                try {
                    const detail = action();
                    steps.push({ label, status: 'success', duration: Math.round(performance.now() - start), detail });
                    return detail;
                } catch (error) {
                    steps.push({ label, status: 'error', duration: Math.round(performance.now() - start), detail: error.message });
                    console.error(`诊断步骤失败: ${label}`, error);
                    return null;
                }
            };

            const startupState = runStep('脚本入口校验', () => {
                const lifecycle = this.state.lifecycle || lifecycleSnapshot;
                return {
                    readyState: document.readyState,
                    hasBody: !!document.body,
                    initialized: lifecycle.initialized,
                    startTriggered: lifecycle.startTriggered,
                    lastStartAttempt: lifecycle.lastStartAttempt ? new Date(lifecycle.lastStartAttempt).toLocaleTimeString() : '未记录'
                };
            });

            const configState = runStep('配置与存储加载', () => ({
                configLoaded: !!this.config,
                pluginSettingsLoaded: !!this.pluginSettings,
                playbackRateDefault: this.config?.playbackRate?.default
            }));

            const modulesState = runStep('核心模块装配', () => {
                const lifecycle = this.state.lifecycle || lifecycleSnapshot;
                ...
                const missingModules = requiredModules.filter(name => !this.managers?.[name]);
                return {
                    modulesReady: lifecycle.modulesReady,
                    missingModules,
                    registered: this.moduleRegistry.list()
                };
            });

            const listenerState = runStep('事件与观察器', () => {
                this.uiController?.createObserver();
                const uiObserverActive = !!this.uiController?.observer;

                if (this.state.lifecycle) {
                    this.state.lifecycle.uiObserverActive = uiObserverActive;
                }

                const lifecycle = this.state.lifecycle || lifecycleSnapshot;
                return {
                    listenersReady: lifecycle.listenersReady,
                    uiObserverActive,
                    uiSubscribers: this.uiController?.subscribers?.size || 0
                };
            });

            const videos = runStep('扫描视频节点', () => Array.from(document.querySelectorAll('video')));
            const panel = runStep('控制面板检测', () => document.getElementById('myPbrMain'));
            runStep('控制面板准备度', () => {
                const lifecycle = this.state.lifecycle || lifecycleSnapshot;
                return {
                    controlsAttempted: lifecycle.controlsAttempted,
                    controlsReady: lifecycle.controlsReady,
                    panelExists: !!panel
                };
            });
            runStep('控制面板重建', () => {
                ...
                this.state.isControlsCreated = false;
                this._main();
                return document.getElementById('myPbrMain') ? '已尝试重建控制面板' : '重建失败，需进一步检查';
            });

            const performanceReport = runStep('性能报告汇总', () => {
                return typeof this.getPerformanceReport === 'function' ? this.getPerformanceReport() : null;
            });

            const missingDependencies = runStep('依赖完整性检查', () => {
                return [
                    'GM_download',
                    'GM_notification',
                    'GM_setValue',
                    'GM_getValue',
                    'GM_xmlhttpRequest'
                ].filter(dep => typeof globalThis[dep] === 'undefined');
            });
            ...
            const diagnostics = {
                ...
                hasBody: !!document.body,
                lifecycle: { ...(this.state.lifecycle || lifecycleSnapshot), uiObserverActive: listenerState?.uiObserverActive },
                videoCount: videos?.length || 0,
                firstVideoMetadata: videos && videos[0] ? {
                    duration: videos[0].duration,
                    readyState: videos[0].readyState,
                    paused: videos[0].paused
                } : null,
                panelExists: !!panel,
                uiState: this.uiController?.uiState,
                registeredModules: modulesState?.registered || [],
                missingCoreModules: modulesState?.missingModules || [],
            ...
            const summary = [
                `文档状态: ${diagnostics.documentReady}`,
                `启动流程: ${diagnostics.lifecycle.initialized ? '初始化完成' : '未触发'}，重试 ${diagnostics.lifecycle.retryCount || 0} 次`,
                `监听器: ${diagnostics.lifecycle.listenersReady ? '已绑定' : '待绑定'} / 观察器: ${diagnostics.lifecycle.uiObserverActive ? '活跃' : '未启动'}`,
                `视频节点: ${diagnostics.videoCount}`,
                `控制面板: ${diagnostics.panelExists ? '已创建' : '缺失'}, 尝试渲染: ${diagnostics.lifecycle.controlsAttempted ? '是' : '否'}`,
                `核心模块缺失: ${diagnostics.missingCoreModules.length ? diagnostics.missingCoreModules.join(', ') : '无'}`,
                `缺失依赖: ${diagnostics.missingDependencies.length ? diagnostics.missingDependencies.join(', ') : '无'}`
            ].join('\n');

            this._renderDiagnosticsOverlay(diagnostics, steps);

            console.groupCollapsed('H5媒体播控器 - 故障诊断');
            ...
            if (typeof GM_notification !== 'undefined') {
                GM_notification({
                    title: 'H5媒体播控器 - 故障诊断',
                    text: `${diagnostics.panelExists ? '控制面板正常' : '控制面板缺失'}，详情见诊断浮层`,
                    timeout: 5000
                });
            } else {
                alert(summary);
        ...
        _renderDiagnosticsOverlay(diagnostics, steps) {
            const existing = document.getElementById('h5-diagnostics-overlay');
            if (existing) existing.remove();

            const overlay = document.createElement('div');
            overlay.id = 'h5-diagnostics-overlay';
            ...
            const formatDetail = (detail) => {
                ...
                if (Array.isArray(detail)) {
                    return detail.length ? detail.join(', ') : '无条目';
                }
                if (typeof detail === 'object') {
                    return Object.entries(detail)
                        .map(([key, value]) => `${key}: ${Array.isArray(value) ? (value.length ? value.join(', ') : '无') : typeof value === 'object' ? JSON.stringify(value) : value}`)
                        .join(' | ');
                }
            ...
            summaryBlock.innerHTML = `
                <div>文档状态：${diagnostics.documentReady}</div>
                <div>视频节点：${diagnostics.videoCount}</div>
                <div style="color:${diagnostics.panelExists ? '#10b981' : '#f97316'};font-weight:600;">
                    控制面板：${diagnostics.panelExists ? '正常' : '未检测到'}
                </div>
                <div>启动流程：${diagnostics.lifecycle?.initialized ? '已执行' : '未触发'}，重试 ${diagnostics.lifecycle?.retryCount || 0} 次</div>
                <div>监听器：${diagnostics.lifecycle?.listenersReady ? '已绑定' : '未绑定'}，观察器：${diagnostics.lifecycle?.uiObserverActive ? '工作中' : '未启动'}（${diagnostics.lifecycle?.controlsAttempted ? '已尝试渲染面板' : '未尝试渲染'}）</div>
                <div>核心模块：${diagnostics.missingCoreModules?.length ? diagnostics.missingCoreModules.join(', ') : '全部加载'}</div>
                <div>缺失依赖：${diagnostics.missingDependencies?.length ? diagnostics.missingDependencies.join(', ') : '无'}</div>
            `;
            ...
            steps.forEach(step => {
                ...
                row.style.cssText = `
                    ...
                    gap: 2px;
                    font-size: 12px;
                    color: ${step.status === 'success' ? '#d1d5db' : '#fca5a5'};
                `;
                const detailText = formatDetail(step.detail);
                row.innerHTML = `
                    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
                        <span>${step.label}${step.status === 'error' ? '（失败）' : ''}</span>
                        <span>${step.duration} ms</span>
                    </div>
                    ${detailText ? `<div style="color:#9ca3af;font-size:11px;">${detailText}</div>` : ''}
                `;
        ...
        _loadPluginSettings() {
            try {
                const stored = GM_getValue('plugin_settings', '{}');
                ...
                console.warn('插件设置读取失败，将使用默认值', error);
                return { siteRules: [] };
            }
        }

        _savePluginSettings(settings) {
            try {
                GM_setValue('plugin_settings', JSON.stringify(settings));
                this.pluginSettings = settings;
                if (typeof GM_notification !== 'undefined') {
                    GM_notification({
                        title: 'H5媒体播控器 - 插件设置',
                        text: '设置已保存，规则更新后请刷新页面生效',
                        timeout: 4000
                    });
                }
            } catch (error) {
                console.error('保存插件设置失败:', error);
                alert('保存插件设置失败，请查看控制台日志');
            }
        }
        ...
        _openSettingsPanel() {
            ...
            header.innerHTML = `
                <div>
                    <div style="font-size:16px;font-weight:700;">插件设置</div>
                    <div style="font-size:12px;color:#9ca3af;">多媒体识别、资源嗅探下载、截图等规则维护</div>
                </div>
            `;
            ...
            helper.innerHTML = `
                ...
                <ul style="margin:0; padding-left:16px;">
                    <li>每条规则描述一个网站的多媒体识别、资源嗅探下载、截图策略。</li>
                    <li>支持字段：<code>domain</code>（必填）、<code>mediaSelectors</code>、<code>downloadHints</code>、<code>screenshot</code>。</li>
                    <li>点击“添加示例规则”可以快速插入模板，然后根据需要修改或删除。</li>
                    <li>保存后会写入油猴存储，刷新页面即可让新规则生效。</li>
                </ul>
            `;
            ...
            textarea.style.cssText = `
                ...
                resize: vertical;
            `;
            textarea.value = JSON.stringify(this.pluginSettings.siteRules, null, 2);
            ...
            addExampleBtn.textContent = '添加示例规则';
            addExampleBtn.style.cssText = `
                background: #1f2937;
                color: #e5e7eb;
            ...
            addExampleBtn.addEventListener('click', () => {
                const rules = this._parseRulesFromTextarea(textarea);
                if (!rules) return;
                rules.push({
                    domain: 'example.com',
                    mediaSelectors: ['video', 'audio'],
                    downloadHints: ['m3u8', 'mp4', 'dash'],
                    screenshot: { mode: 'video', captureControls: true }
                });
            ...
            saveBtn.addEventListener('click', () => {
                const rules = this._parseRulesFromTextarea(textarea);
                if (!rules) return;
                this._savePluginSettings({ ...this.pluginSettings, siteRules: rules });
                overlay.remove();
            });

            const resetBtn = document.createElement('button');
            resetBtn.textContent = '清空规则';
            ...
            actions.append(addExampleBtn, resetBtn, saveBtn);
            body.append(helper, textarea, actions);
        ...
        _parseRulesFromTextarea(textarea) {
            try {
        ...
        setupVideoPlaybackListener() {
            ...
            document.addEventListener('ended', (event) => {
                if (event.target.tagName === 'VIDEO') {
                    ...
                    try {
                        // 如果视频设置为循环播放，则清除保存的播放记录，避免循环自动播放时恢复到末尾位置
                        if (event.target.loop) {
                            if (this.controller && this.controller.managers && this.controller.managers.storage) {
                                this.controller.managers.storage.clearPlaybackRecord(event.target);
                            }
                            if (this.controller && this.controller.managers && this.controller.managers.restoration) {
                                this.controller.managers.restoration.resetVideoRestorationState(event.target);
                            }
                        }
                    } catch (e) {
                        console.log('处理 ended 事件时出错:', e);
                    }
                }
            }, true);
        }
    ...
    class StreamingMediaEnhancer {
        constructor(controller) {
            ...
            // 使用 Map 以支持迭代统计（WeakMap 无法遍历会导致 TypeError）
            this.activeManagers = new Map();
            this.cleanupIntervalId = null;
            this.performanceIntegrated = false;
         ...
         */
        initialize() {
            this.enabled = this.controller?.state?.isStreamingOptimizationEnabled !== false;

            if (!this.enabled) {
                console.log('流媒体优化已关闭，跳过初始化');
                return;
            }
    ...
    class EnhancedVideoManager {
        ...
        /**
         * 性能模块准备就绪后更新引用并重新应用优化
         */
        onPerformanceModuleReady(performanceAnalyzer, performanceMonitor) {
            ...
            // 根据新的性能数据重新优化当前视频
            document.querySelectorAll('video').forEach(video => this.optimizeVideoPlayback(video));
        }
        ...
        updateControlsState() {
            ...
            const shouldShowControls = !!targetVideo || isAudioMode || hasAudioVideo;

            this.controller.ui.ensureControlsVisible();

            if (shouldShowControls && targetVideo) {
                if (targetVideo !== this.controller.state.currentVideo) {
                    this.controller.managers.batch.debouncedVideoSwitch(targetVideo);
                } else if (!this.controller.state.currentVideo) {
                    this.controller.managers.batch.debouncedVideoSwitch(targetVideo);
                }
            ...
            } else if (!shouldShowControls && this.controller.ui.controlsContainer) {
                this.controller.ui._updateUIState({
                    areControlsVisible: false,
                    isSpeedPanelVisible: false
                });
                this.controller.ui._applyVisibilityState();
                this.controller.setState({
                    currentVideo: null,
                    previousVideo: null
                });
    ...
    class EnhancedStorageManager {
        ...
        savePlaybackRecord(video) {
            ...
            // 如果当前位置非常接近视频结尾（例如距离结尾小于1s），则不保存，防止循环/短视频结束后错误恢复
            try {
                const dur = video.duration || 0;
                if (dur > 0 && video.currentTime >= Math.max(0, dur - 3)) {
                    return;
                }
            } catch (e) {
                // ignore
        ...
        clearPlaybackRecord(video) {
            if (!video) return;
            const recordKey = this.getVideoRecordKey(video);
            try {
                GM_setValue(recordKey, 'null');
                console.log(`已清除播放记录: ${recordKey}`);
            } catch (e) {
                console.log('清除播放记录失败:', e);
            }
        }
    ...
    class EnhancedRestorationManager {
        ...
        shouldRestorePlayback(video, savedRecord) {
            // 如果视频处于循环播放，直接跳过恢复，避免循环时读取旧进度导致跳转到接近末尾的位置
            try {
                if (video && video.loop) {
                    console.log('视频处于循环模式，跳过进度恢复');
                    return false;
                }
            } catch (e) {
                // ignore
            ...
            // 如果保存的位置非常接近视频结尾（例如小于3s差距），视为已看完，跳过恢复
            try {
                const durationVal = video.duration || savedRecord.duration || 0;
                if (durationVal > 0 && savedRecord.currentTime >= Math.max(0, durationVal - 3)) {
                    console.log('保存的播放位置接近结尾，跳过进度恢复');
                    return false;
                }
            } catch (e) {
                // 忽略异常，继续后续恢复判断
            ...
            // 修复循环播放时错误恢复进度的问题（短视频循环跳回末尾）
            try {
                const durationVal2 = video.duration || savedRecord.duration || 0;
                if (video.loop && durationVal2 > 0 && savedRecord.currentTime > Math.max(0, durationVal2 - 3) && video.currentTime < 1) {
                    console.log('检测到循环播放且保存位置在末尾，跳过进度恢复');
                    return false;
                }
            } catch (e) {
                // ignore
    ...
    class ScreenshotManager {
        ...
        _downloadScreenshot(canvas, absoluteTime, index) {
            try {
                ...
                // 异步处理，避免阻塞UI
                requestIdleCallback(() => {
                    this._performScreenshotDownload(canvas, filename, absoluteTime, index);
                }, { timeout: 3000 });
        ...
        _performScreenshotDownload(canvas, filename, absoluteTime, index) {
            // 检测是否为直播流媒体
            const isLiveStream = this.currentVideo && this._isLiveStreamingVideo(this.currentVideo);
            
            console.log('[截图下载] 开始下载，直播标志:', isLiveStream);
            console.log('[截图下载] GM_download 可用:', typeof GM_download !== 'undefined');
            
            canvas.toBlob((blob) => {
                if (blob) {
                    console.log('[截图下载] Blob 生成成功，大小:', (blob.size / 1024).toFixed(2), 'KB');
                    
                    if (isLiveStream && typeof GM_download !== 'undefined') {
                        console.log('[截图下载] 使用 GM_download 方式下载直播截图');
                        // 直播流媒体：使用 GM_download 支持文件下载确认框
                        this._downloadViaGM(blob, filename);
                    } else if (isLiveStream) {
                        console.log('[截图下载] 直播视频但 GM_download 不可用，使用确认弹窗方式');
                        // 直播但没有 GM_download：使用确认弹窗
                        this._downloadViaConfirmDialog(blob, filename);
                    } else {
                        console.log('[截图下载] 使用 File API 方式下载普通视频截图');
                        // 普通视频：使用传统方式
                        this._downloadViaFileAPI(blob, filename);
                    }
                } else {
                    console.error('[截图下载] Canvas转换blob失败');
                    this._showNotification('错误', 'Canvas转换blob失败', 3000);
                    this._closePreview();
                }
            }, 'image/png', 0.95); // 添加质量参数
        }

        /**
         * 通过确认对话框下载（直播视频备选方案）
         */
        _downloadViaConfirmDialog(blob, filename) {
            try {
                console.log('[确认下载] 显示下载确认对话框');
                
                // 创建确认对话框
                const confirmed = confirm(`是否确认下载截图？\n\n文件名: ${filename}\n大小: ${(blob.size / 1024).toFixed(2)} KB\n\n点击"确定"确认下载，点击"取消"放弃。`);
                
                if (confirmed) {
                    console.log('[确认下载] 用户确认，开始下载');
                    this._downloadViaFileAPI(blob, filename);
                } else {
                    console.log('[确认下载] 用户取消下载');
                    this._showNotification('已取消', '下载已取消', 2000);
                    this._closePreview();
                }
            } catch (error) {
                console.error('[确认下载] 异常:', error);
                this._downloadViaFileAPI(blob, filename);
            }
        }
         ...
         * 判断是否为直播流媒体视频
         */
        _isLiveStreamingVideo(videoElement) {
            if (!videoElement) return false;
            
            const src = (videoElement.src || videoElement.currentSrc || '').toLowerCase();
            const sources = videoElement.querySelectorAll('source');
            
            console.log('[直播检测] 当前视频源:', src);
            console.log('[直播检测] 视频duration:', videoElement.duration);
            
            // 检查主源
            if (src.includes('m3u8') || src.includes('.ts') || src.includes('manifest') || src.includes('dash')) {
                console.log('[直播检测] 检测到直播协议: HLS/DASH');
                return true;
            }
            ...
            for (const source of sources) {
                ...
                console.log('[直播检测] 子源:', sourceSrc, '类型:', sourceType);
                
                if (sourceSrc.includes('m3u8') || sourceSrc.includes('.ts') || 
                    sourceSrc.includes('manifest') || sourceSrc.includes('dash') ||
                    sourceType.includes('application/x-mpegURL') || 
                    sourceType.includes('application/dash+xml')) {
                    console.log('[直播检测] 检测到直播子源');
                    return true;
                }
            }
            
            // 检查是否为实时流（无duration 或 duration=Infinity）
            const isLiveByDuration = videoElement.duration === Infinity || 
                                     (isNaN(videoElement.duration) && !isNaN(videoElement.currentTime));
            
            if (isLiveByDuration) {
                console.log('[直播检测] 检测到无duration的实时流');
                return true;
            }
            
            console.log('[直播检测] 判定为普通视频');
            return false;
        }

        /**
         * 通过 GM_download 下载（支持文件确认框）
         */
        _downloadViaGM(blob, filename) {
            try {
                console.log('[GM_download] 开始转换blob为Data URL...');
                
                // 显示正在处理的提示
                this._showNotification('正在处理', '正在准备下载文件...', 2000);
                
                // GM_download 不支持 blob:// URL，需要转换为 Data URL
                const reader = new FileReader();
                
                reader.onload = () => {
                    const dataUrl = reader.result;
                    console.log('[GM_download] 开始下载，文件名:', filename, '大小:', (blob.size / 1024).toFixed(2), 'KB');
                    
                    // 显示即将下载提示
                    this._showNotification('即将下载', `正在请求下载: ${filename}`, 1500);
                    ...
                    try {
                        GM_download({
                            ...
                            onload: () => {
                                console.log(`[GM_download] 下载完成: ${filename}`);
                                this._showNotification('✅ 下载完成', `截图已保存: ${filename}`, 3000);
                                this._closePreview();
                            },
                            onerror: (error) => {
                                console.error(`[GM_download] 下载失败:`, error);
                                const errMsg = error && error.message ? error.message : '未知错误';
                                this._showNotification('⚠️ GM下载失败', `错误: ${errMsg}，尝试其他方式...`, 2000);
                                // 降级到传统方式
                                setTimeout(() => {
                                    this._downloadViaFileAPI(blob, filename);
                                }, 500);
                            ...
                            ontimeout: () => {
                                console.warn(`[GM_download] 下载超时`);
                                this._showNotification('⏱️ 下载超时', '正在尝试其他方式...', 2000);
                                setTimeout(() => {
                                    this._downloadViaFileAPI(blob, filename);
                                }, 500);
                            }
                        ...
                        console.error('[GM_download] 调用异常:', gmError);
                        this._showNotification('⚠️ 调用异常', '正在使用备用方式...', 2000);
                        setTimeout(() => {
                            this._downloadViaFileAPI(blob, filename);
                        }, 500);
                    }
                ...
                reader.onerror = () => {
                    console.error('[GM_download] FileReader 失败');
                    this._showNotification('⚠️ 文件读取失败', '正在使用其他方式...', 2000);
                    setTimeout(() => {
                        this._downloadViaFileAPI(blob, filename);
                    }, 500);
                };
                
                console.log('[GM_download] 开始读取blob...');
                reader.readAsDataURL(blob);
                
            } catch (error) {
                console.error('GM_download 异常:', error);
                this._showNotification('⚠️ 异常', '正在使用备用下载...', 2000);
                setTimeout(() => {
                    this._downloadViaFileAPI(blob, filename);
                }, 500);
            }
         ...
         * 通过 File API 下载（无确认框）
         */
        _downloadViaFileAPI(blob, filename) {
            try {
                console.log('[File API] 开始通过 File API 下载:', filename);
                this._showNotification('下载中', '正在触发浏览器下载...', 1500);
                ...
                setTimeout(() => {
                    try {
                        ...
                        console.log('[File API] 资源已清理');
                    } catch (e) {
                        console.warn('[File API] 清理资源异常:', e);
                    }
                ...
                console.error('[File API] 下载失败:', error);
                this._showNotification('⚠️ File API失败', '正在尝试降级方法...', 2000);
                setTimeout(() => {
        ...
        _fallbackDownload(blob, filename) {
            try {
                console.log('[降级下载] 开始降级下载，blob大小:', (blob.size / 1024).toFixed(2), 'KB');
                
                // 方法1：尝试 DataURL（仅适合小文件）
                if (blob.size < 5 * 1024 * 1024) { // 5MB 限制
                    console.log('[降级下载] 使用 DataURL 方式');
                    this._showNotification('降级下载', '正在使用 DataURL 方式...', 1500);
                    
                    const reader = new FileReader();
                    reader.onload = () => {
                        const dataUrl = reader.result;
                        console.log('[降级下载] DataURL 生成成功');
                    ...
                    reader.onerror = () => {
                        console.error('[降级下载] FileReader 读取失败');
                        throw new Error('FileReader 失败');
                    };
                    reader.readAsDataURL(blob);
                } else {
                    console.error('[降级下载] 文件过大，无法使用 DataURL 降级');
                    throw new Error('文件过大，无法使用 DataURL 降级');
                }
            } catch (error) {
                console.error('[降级下载] 所有下载方法失败:', error);
                this._showNotification('❌ 下载失败', '无法下载图片，请检查浏览器设置', 3000);
                this._closePreview();
            }
        }
    ...
    class EnhancedUIManager {
        constructor(controller, uiController) {
            ...
            if (this.uiController) {
                this._unsubscribe = this.uiController.onVideoChange((newState) => {
                    this._updateUIState({
                        areControlsVisible: !!newState.hasActiveVideo,
                        isSpeedPanelVisible: newState.isPanelVisible && !!newState.hasActiveVideo
                    });
    ...
    `;
        ...
        _updateUIState(partialState) {
            const controllerUpdates = {};

            Object.entries(partialState).forEach(([key, value]) => {
                if (this.uiState[key] === value) return;

                this.uiState[key] = value;

                if (['isSpeedPanelVisible', 'areControlsVisible'].includes(key)) {
                    controllerUpdates[key] = value;
                }
            });

            if (Object.keys(controllerUpdates).length > 0) {
                this.controller.setState(controllerUpdates);
            }
        }

        _applyVisibilityState() {
            if (this.controlsContainer) {
                this.controlsContainer.style.display = this.uiState.areControlsVisible ? 'flex' : 'none';
            }

            if (this.speedPanel) {
                const shouldShowPanel = this.uiState.isSpeedPanelVisible && this.uiState.areControlsVisible;
                this.speedPanel.classList.toggle("show", shouldShowPanel);
                this.speedPanel.classList.toggle("hidden", !shouldShowPanel);
                this.speedPanel.style.display = shouldShowPanel ? 'flex' : 'none';

                if (!shouldShowPanel && this.captureBtn) {
                    this.captureBtn.style.display = 'none';
                }
            }
        }

        _startVisibilityReconciler() {
            ...
            this._visibilityObserver = new MutationObserver(() => this._applyVisibilityState());
            this._visibilityObserver.observe(this.controlsContainer, {
                childList: true,
                subtree: true,
                attributes: true,
        ...
        createControls() {
            ...
            if (this.uiController) {
                this._updateUIState({
                    isSpeedPanelVisible: this.uiController.uiState.isPanelVisible && this.uiController.uiState.hasActiveVideo,
                    areControlsVisible: this.uiController.uiState.hasActiveVideo
                });
            } else {
                this._updateUIState({
                    isSpeedPanelVisible: this.controller.state.isSpeedPanelVisible,
                    areControlsVisible: this.controller.state.areControlsVisible
                });
            }
            this._startVisibilityReconciler();
        }
        ...
        _toggleSpeedPanel() {
            ...
            if (this.uiState.isSpeedPanelVisible) {
                this.hideSpeedPanel();
            } else {
                this.showSpeedPanel();
            }
        ...
        showSpeedPanel() {
            if (!this.speedPanel || this.uiState.isTransitioning) return;

            this._beginTransitionGuard();

            const targetVideo = this.controller.managers.video.getTargetVideo();
            if (targetVideo && this._canCaptureVideo(targetVideo)) {
                this.captureBtn.style.display = 'flex';
            } else if (this.captureBtn) {
                this.captureBtn.style.display = 'none';
            }
            ...
            if (this.uiController) {
                this.uiController.updateState({ isPanelVisible: true });
            } else {
                this._updateUIState({
                    isSpeedPanelVisible: true,
        ...
        ensureControlsVisible() {
            if (this.uiController) {
                this.uiController.updateState({ hasActiveVideo: true });
            }
        }
        ...
        setMainBtnTxt() {
            if (this.mainBtn) {
                const targetVideo = this.controller.managers.video.getTargetVideo();
                const isLiveStream = targetVideo && (!isFinite(targetVideo.duration) || targetVideo.duration === Infinity || targetVideo.duration === 0);
                if (isLiveStream) {
                    this.mainBtn.innerHTML = 'Live';
                    return;
                }
        ...
        hideSpeedPanel() {
            ...
            if (this.uiController) {
                this.uiController.updateState({ isPanelVisible: false });
            } else {
                this._updateUIState({ isSpeedPanelVisible: false });
                this._applyVisibilityState();
            }
        }
    }
