    ...
    class VideoSpeedController {
        ...
        _notifyStreamingStatus(title, text) {
            if (typeof GM_notification !== 'undefined') {
                ...
                console.log(`H5媒体播控器 - ${title}: ${text}`);
            }
        }

        _runDiagnostics() {
            const panel = document.getElementById('myPbrMain');
            const videos = Array.from(document.querySelectorAll('video'));
            const performanceReport = typeof this.getPerformanceReport === 'function'
                ? this.getPerformanceReport()
                : null;
            const registeredModules = this.moduleRegistry.list();
            ...
            const summary = [
                ...
                `模块: ${registeredModules.length ? registeredModules.join(', ') : '无'}`,
                `缺失依赖: ${missingDependencies.length ? missingDependencies.join(', ') : '无'}`
            ].join('\n');
            ...
            if (typeof GM_notification !== 'undefined') {
                GM_notification({
                    title: 'H5媒体播控器 - 故障诊断',
                    text: `${diagnostics.panelExists ? '控制面板正常' : '控制面板缺失'}，详情见控制台`,
                    timeout: 5000
                });
            } else {
                alert(summary);
